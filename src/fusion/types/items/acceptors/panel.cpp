...
PanelItemAcceptor::PanelItemAcceptor(Spatial *parent, Field &field, Vec3 origin,
                                     Quat rotation)
    : ItemAcceptor(parent, field, origin, rotation, "panel") {}

std::vector<uint8_t>
PanelItemAcceptor::acceptorCallback(flexbuffers::Reference data, bool) {
  ...
  if (captured)
    itemCapturedMethod(
        item, PanelItem::Data{dataVec[0].AsUInt32(), dataVec[1].AsUInt32()});
  else
