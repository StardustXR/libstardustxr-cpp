...
public:
  explicit Spatial();
  explicit Spatial(Spatial *parent, Vec3 origin = Vec3::Zero,
                   Quat rotation = Quat::Identity, Vec3 scale = Vec3::One,
                   bool translatable = true, bool rotatable = true,
                   bool scalable = true, bool zoneable = false);

  void createLaunchAnchor(std::function<void(uint32_t)> callback);

  void getTransform(Spatial *space,
