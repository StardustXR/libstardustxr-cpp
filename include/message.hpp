...
struct Message FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
	...
	bool Verify(flatbuffers::Verifier &verifier) const {
		return VerifyTableStart(verifier) && VerifyField<uint8_t>(verifier, VT_TYPE) && VerifyField<uint32_t>(verifier, VT_ID) && VerifyOffset(verifier, VT_OBJECT) && verifier.VerifyString(object()) && VerifyOffset(verifier, VT_METHOD) && verifier.VerifyString(method()) && VerifyOffset(verifier, VT_ERROR) && verifier.VerifyString(error()) && VerifyOffset(verifier, VT_DATA) && verifier.VerifyVector(data()) && verifier.EndTable();
	}
};
...
inline flatbuffers::Offset<Message> CreateMessageDirect(
	...
	return StardustXR::CreateMessage(
			_fbb,
			type,
			id,
			object__,
